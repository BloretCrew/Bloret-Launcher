...
Dialog {
    ...
    Component {
        ...
        Button {
            text: Backend ? Backend.tr("询问 Blora Agent") : "询问 Blora Agent"
            highlighted: true
            Layout.alignment: Qt.AlignRight
            Layout.preferredWidth: footer ? footer.availableWidth / 2 : 120
            onClicked: {
                var prompt = buildBlorikoPrompt()
                console.log("[ErrorAnalysis] 请求 Blora Agent 分析，提示词长度:", prompt.length)
    ...
    function analyzeError(text) {
        ...
        if (text.indexOf("OutOfMemoryError") !== -1) {
            tips.push((Backend ? Backend.tr("检测到内存不足错误 (OutOfMemoryError)。建议在启动设置中增加 JVM 最大内存分配（如 -Xmx4G）。") : "检测到内存不足错误 (OutOfMemoryError)。建议在启动设置中增加 JVM 最大内存分配（如 -Xmx4G）。"))
        }

        if (text.indexOf("ClassNotFoundException") !== -1 || text.indexOf("NoClassDefFoundError") !== -1) {
            tips.push((Backend ? Backend.tr("检测到类未找到错误。这表示缺少必要的库文件。建议重新下载该版本的核心文件，或检查是否有 Mod 依赖缺失。") : "检测到类未找到错误。这表示缺少必要的库文件。建议重新下载该版本的核心文件，或检查是否有 Mod 依赖缺失。"))
        }

        if (text.indexOf("NoSuchMethodError") !== -1) {
            tips.push((Backend ? Backend.tr("检测到方法未找到错误。这通常表示 Mod 与当前 Minecraft 版本不兼容。建议检查 Mod 是否适配当前游戏版本。") : "检测到方法未找到错误。这通常表示 Mod 与当前 Minecraft 版本不兼容。建议检查 Mod 是否适配当前游戏版本。"))
        }

        if (text.indexOf("ConcurrentHashMap") !== -1 || text.indexOf("ConcurrentModification") !== -1) {
            tips.push((Backend ? Backend.tr("检测到并发修改错误。这可能是由于 Mod 的线程安全问题或 Java 版本不兼容导致的。建议尝试更换 Java 版本或更新相关 Mod。") : "检测到并发修改错误。这可能是由于 Mod 的线程安全问题或 Java 版本不兼容导致的。建议尝试更换 Java 版本或更新相关 Mod。"))
        }

        if (text.indexOf("hs_err_pid") !== -1 || text.indexOf("EXCEPTION_ACCESS_VIOLATION") !== -1) {
            tips.push((Backend ? Backend.tr("检测到 JVM 崩溃。这通常与 Java 版本、显卡驱动或 Mod 的 native 库有关。建议更新显卡驱动、更换 Java 版本或检查 Mod 的 native 依赖。") : "检测到 JVM 崩溃。这通常与 Java 版本、显卡驱动或 Mod 的 native 库有关。建议更新显卡驱动、更换 Java 版本或检查 Mod 的 native 依赖。"))
        }

        if (text.indexOf("java.lang.reflect") !== -1 || text.indexOf("IllegalAccess") !== -1) {
            tips.push((Backend ? Backend.tr("检测到反射访问错误。这可能是由于 Java 模块系统限制导致的。建议更换 Java 版本（推荐 Java 8 或 Java 17）。") : "检测到反射访问错误。这可能是由于 Java 模块系统限制导致的。建议更换 Java 版本（推荐 Java 8 或 Java 17）。"))
        }

        if (text.indexOf("GLFW") !== -1 || text.indexOf("OpenGL") !== -1 || text.indexOf("LWJGL") !== -1) {
            tips.push((Backend ? Backend.tr("检测到图形库错误。这通常与显卡驱动或 LWJGL 版本有关。建议更新显卡驱动，或在设置中尝试更换 LWJGL 版本。") : "检测到图形库错误。这通常与显卡驱动或 LWJGL 版本有关。建议更新显卡驱动，或在设置中尝试更换 LWJGL 版本。"))
        }

        if (text.indexOf("SecurityException") !== -1) {
            tips.push((Backend ? Backend.tr("检测到安全异常。可能是安全软件（如杀毒软件）阻止了 Minecraft 运行。建议将启动器和游戏目录添加到安全软件的白名单中。") : "检测到安全异常。可能是安全软件（如杀毒软件）阻止了 Minecraft 运行。建议将启动器和游戏目录添加到安全软件的白名单中。"))
        }

        if (text.indexOf("FileNotFoundException") !== -1 || text.indexOf("ENOENT") !== -1) {
            tips.push((Backend ? Backend.tr("检测到文件缺失错误。关键游戏文件可能已损坏或被删除。建议在下载页面重新下载该版本的核心文件。") : "检测到文件缺失错误。关键游戏文件可能已损坏或被删除。建议在下载页面重新下载该版本的核心文件。"))
        }

        if (tips.length === 0) {
            return (Backend ? Backend.tr("游戏启动过程中遇到未知错误。你可以复制错误信息并向社区寻求帮助。") : "游戏启动过程中遇到未知错误。你可以复制错误信息并向社区寻求帮助。")
        }
